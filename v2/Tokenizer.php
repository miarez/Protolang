...
class Tokenizer {
    ...
    private function extractNestedExpressions(
        array $tokens
    ) : array
    {
        ...
        while ($tokenIndex < count($tokens)) {
            ...
            if ($token === "{")
            {
                ...
                $tokens = $this->bindTillClosureToken($tokens, $tokenIndex, "}");
            }
        ...
        while ($queryCharPosition < strlen($dslQuery)) {
            ...
            for ($tokenSelectorPosition = $this->tokenSize; $tokenSelectorPosition > 0; $tokenSelectorPosition--)
            {
                ...
                if(in_array($selection, self::SPLITTERS))
                {
                    ...
                    # the reason a token would already exist is the current character is a splitter and some characters
                    # had already been appended to the token string
